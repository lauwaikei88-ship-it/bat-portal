...
export async function postToFacebookPage(
  ...
  if (formatType === 'CAROUSEL' && mediaUrls.length > 1) {
    ...
    for (const url of mediaUrls) {
      ...
      const photoRes = await fetch(`${META_GRAPH_URL}/${fbPageId}/photos`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams(photoParams),
      });
      const photoData = await photoRes.json();
      if (!photoData?.id) throw new Error(`Failed to upload unpublished Facebook photo: ${JSON.stringify(photoData)}`);
    ...
    const feedRes = await fetch(`${META_GRAPH_URL}/${fbPageId}/feed`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams(feedParams),
    });
    const feedData = await feedRes.json();
    if (!feedData?.id) throw new Error(`Failed to publish Facebook carousel: ${JSON.stringify(feedData)}`);
    return feedData.id;
  }

  if (formatType === 'STORY') {
    if (isVideo) {
      const publishRes = await fetch(`${META_GRAPH_URL}/${fbPageId}/video_stories`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ file_url: url, access_token: accessToken }),
      });
      const publishData = await publishRes.json();
      if (!publishData?.id) throw new Error(`Failed to publish Facebook video story: ${JSON.stringify(publishData)}`);
      return publishData.id;
    } else {
      const publishRes = await fetch(`${META_GRAPH_URL}/${fbPageId}/photo_stories`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ url: url, access_token: accessToken }),
      });
      const publishData = await publishRes.json();
      if (!publishData?.id && !publishData?.post_id) throw new Error(`Failed to publish Facebook photo story: ${JSON.stringify(publishData)}`);
      return publishData.id || publishData.post_id;
    }
  }
